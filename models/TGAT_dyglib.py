...
class TGAT(nn.Module):
    ...
    def __init__(self, node_raw_features: np.ndarray, edge_raw_features: np.ndarray, neighbor_sampler: NeighborFinder,
    ...
    def forward(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray,edge_ids:np.ndarray,
        ...
        src_node_embeddings,_ = self.compute_node_temporal_embeddings(node_ids=src_node_ids, node_interact_times=node_interact_times,\
                                                                    current_layer_num=self.num_layers, num_neighbors=num_neighbors)
        # Tensor, shape (batch_size, node_feat_dim)
        dst_node_embeddings,_ = self.compute_node_temporal_embeddings(node_ids=dst_node_ids, node_interact_times=node_interact_times,\
                                                                    current_layer_num=self.num_layers, num_neighbors=num_neighbors)
        return src_node_embeddings, dst_node_embeddings

    def forward_with_neighbors(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray,edge_ids:np.ndarray,
                                                    node_interact_times: np.ndarray, num_neighbors: int = 20):  
        """
        compute source and destination node temporal embeddings
        :param src_node_ids: ndarray, shape (batch_size, )
        ...
        """
        # Tensor, shape (batch_size, node_feat_dim)
        src_node_embeddings,src_neighbors = self.compute_node_temporal_embeddings(node_ids=src_node_ids, node_interact_times=node_interact_times,
                                                                    current_layer_num=self.num_layers, num_neighbors=num_neighbors)
        # Tensor, shape (batch_size, node_feat_dim)
        dst_node_embeddings,_ = self.compute_node_temporal_embeddings(node_ids=dst_node_ids, node_interact_times=node_interact_times,
                                                                    current_layer_num=self.num_layers, num_neighbors=num_neighbors)
        
        return src_node_embeddings, dst_node_embeddings,src_neighbors
    ...
    def compute_node_temporal_embeddings(self, node_ids: np.ndarray, node_interact_times: np.ndarray,
        ...
        # node_interact_times = np.array(node_interact_times,dtype=np.int64)
        ...
        # Tensor, shape (batch_size, node_feat_dim)
        node_raw_features = self.node_raw_features[torch.from_numpy(node_ids).long()]

        if current_layer_num == 0:
            return node_raw_features, None
        else:
            # get source node representations by aggregating embeddings from the previous (current_layer_num - 1)-th layer
            # Tensor, shape (batch_size, node_feat_dim)
            node_conv_features,_ = self.compute_node_temporal_embeddings(node_ids=node_ids,
            ...
            neighbor_node_ids, neighbor_edge_ids, neighbor_times = \
                self.neighbor_sampler.get_historical_neighbors(node_ids,
                                                               node_interact_times,
                                                               num_neighbors)
            ...
            neighbor_node_conv_features,_ = self.compute_node_temporal_embeddings(node_ids=neighbor_node_ids.flatten(),
            ...
            # print('***************** src_ngh_t_batch_th',neighbor_delta_times)
            ...
            return output,neighbor_node_conv_features

    def set_neighbor_finder(self, neighbor_sampler: NeighborFinder):
        """

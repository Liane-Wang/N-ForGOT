...
def eval_prediction(model, data, task, eval_task, bs, ch, uml, avg='edge', multihead=True, per_class=3, retrain=False):
    ...
    with torch.no_grad():
        ...
        for i in range(num_batch):
            ...
            src_logits, dst_logits, loss = model.get_logits(src_batch, dst_batch, edge_batch, timestamps_batch, model.args.num_neighbors, task)
            ...
            src_ap=precision_score(src_true_label, src_pred_label, labels=[i for i in range(0, n_class)], average='macro')
            dst_ap=precision_score(dst_true_label, dst_pred_label, labels=[i for i in range(0, n_class)], average='macro')
        ...
        if ch=='test':
            ...
            #     return true_acc,true_acc,true_acc, np.mean(np.array(list(node_mp2.values()))*100), src_logits, dst_logits
            # else:
            return true_acc,np.mean(val_acc), np.mean(val_ap), np.mean(val_f1), loss , 'test'
        ...
        if avg == 'node':
            ...
            #     return np.mean(node_pred), np.mean(node_pred), np.mean(node_pred), np.mean(node_pred2), src_logits, dst_logits
            return np.mean(node_pred),np.mean(val_acc), np.mean(val_ap), np.mean(val_f1),loss,'node'
        
    return 0, np.mean(val_acc), np.mean(val_ap), np.mean(val_f1) , loss ,'else'
